...
- **Context window visualizer** — live bar showing how full your 200k context window is, broken down by system prompt, history, and last message
- **Cost projector** — drag a slider to set monthly call volume, instantly see monthly and yearly cost with model comparisons
- **Prompt optimizer** — paste your system prompt, get a token-reduced version generated by Haiku, apply with one click
- **Model router** — analyzes your last 10 messages and suggests the right model tier based on query complexity
...
| **Insights ✦** | Toggle the three insight panels: cost projector, prompt optimizer, and model router. |

---

## Insight panels

Three panels appear at the bottom of the screen, toggled with the **insights ✦** button.

**Context window visualizer**
A live bar always visible below the config showing how full your 200,000 token context window is. Color-coded segments show system prompt tokens (coral), conversation history (green), and your current message (blue). Turns red as you approach limits.

**Cost projector**
Set a monthly call volume with a slider and instantly see projected monthly and yearly cost. Also shows what the same volume would cost on Haiku, with the savings amount.

**Prompt optimizer**
Analyzes your system prompt and rewrites it using Haiku to use fewer tokens while preserving all meaning. Shows before/after token count and lets you apply or discard the result with one click. Only costs a small Haiku call.

**Model router**
Analyzes your last 10 messages and scores each one by complexity — simple queries that could run on Haiku, mid-complexity for Sonnet, and hard reasoning tasks that need Opus. Shows a percentage breakdown and a recommendation. Updates after every message.
